...
class ViewNotePage extends StatefulWidget {
  const ViewNotePage({
    super.key,
    required this.title,
    required this.quillControllerData,
    required this.indexData,
    required this.noteTitle,
  });

  final QuillController quillControllerData;
  final int indexData;
...
class _ViewNotePageState extends State<ViewNotePage> {
  late QuillController _quillscontroller = QuillController.basic();
  late TextEditingController _textEditingController;
  final noteBox = objectbox.store.box<Note>();
  late bool noteSaved = false;
  late bool initDocData = true; //Sets the initial data for quillcontroller
  late Note note;

  //Saves and updates the note based on if the note was previously saved or not
  void _saveNote(int indexData) {
    if (mounted) {
      setState(() {
  ...
  Widget build(BuildContext context) {
    if (initDocData) {
      _quillscontroller.document = widget.quillControllerData.document;
      _textEditingController.text = widget.noteTitle;
      initDocData = false;
    }

    final int indexData = widget.indexData;
        ...
        child: Column(
          ...
          children: <Widget>[
            Padding(
              ...
              child: TextFormField(
                controller: _textEditingController, //needs to be changed
